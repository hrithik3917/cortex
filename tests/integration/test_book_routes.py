...
def auth_token(client):
    ...
    client.post("/v1/users/", json={"email": TEST_EMAIL, "password": TEST_PASSWORD})

    # Login — this always gives us a fresh token
    response = client.post(
        "/v1/auth/login", data={"username": TEST_EMAIL, "password": TEST_PASSWORD}
    )
...
def test_create_book_without_auth_returns_401(client):
    response = client.post(
        "/v1/books/", json={"title": "Should Fail", "author": "Author", "pages": 100}
    )
...
def test_title_returns_400_with_correct_code(client, auth_headers):
    ...
    first = client.post(
        "/v1/books/",
        json={"title": title, "author": "Author", "pages": 100},
        headers=auth_headers,
    )
...
def test_edit_other_users_book_returns_403_with_correct_code(client, auth_headers):
    ...
    create_response = client.post(
        "/v1/books/",
        json={"title": f"Owner Book {UNIQUE_PREFIX}", "author": "Owner", "pages": 100},
        headers=auth_headers,
    )

    assert create_response.status_code == 201, (
        f"Book creation failed: {create_response.json()}"
    )
    ...
    signup_response = client.post(
        "/v1/users/", json={"email": other_email, "password": "testpass123"}
    )
    ...
    login_response = client.post(
        "/v1/auth/login", data={"username": other_email, "password": "testpass123"}
    )
    ...
    response = client.put(
        f"/v1/books/{book_id}", json={"title": "Hacked Title"}, headers=other_headers
    )
