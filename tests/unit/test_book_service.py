...
def test_fetch_all_books_queries_db_on_cache_miss(mock_db):
    ...
    with (
        patch(
            "app.services.book_service.get_cached_books",
            return_value=None,
        ),
        patch(
            "app.services.book_service.get_all_books",
            return_value=([fake_book], 1),
        ),
        patch("app.services.book_service.set_cached_books"),
        patch.object(
            BookResponse,
            "model_validate",
            return_value=BookResponse(
                id=1,
                title="Clean Code",
                author="Martin",
                pages=464,
...
def test_fetch_all_books_pagination_math_is_correct(mock_db):
    ...
    ]  # This created 10 seperate MagicMock objects
    ...
    with (
        ...
        patch(
            "app.services.book_service.get_all_books",
            return_value=(fake_books, 25),
        ) as mock_query,
        patch("app.services.book_service.set_cached_books"),
        patch.object(
            BookResponse,
            "model_validate",
            side_effect=lambda b: BookResponse(
                id=b.id,
                title=b.title,
                author=b.author,
                pages=b.pages,
...
def test_fetch_book_raises_404_when_not_in_cache_or_db(mock_db):
    ...
    with (
        patch(
            "app.services.book_service.get_cached_book",
            return_value=None,
        ),
        patch(
            "app.services.book_service.get_book_by_id",
            return_value=None,
        ),
...
def test_insert_book_raises_400_on_duplicate_title(mock_db, mock_user):
    ...
    with (
        patch("app.services.book_service.insert_book", return_value=fake_exisitng),
        pytest.raises(HTTPException) as exc_info,
    ):
...
def test_insert_book_succeeds_when_title_is_unique(mock_db, mock_user):
    ...
    with (
        patch(
            "app.services.book_service.get_book_by_title",
            return_value=None,
        ),
        patch("app.services.book_service.insert_book_model", return_value=fake_book),
        patch("app.services.book_service.invalidate_book"),
    ):
...
def test_modify_book_succeeds_when_user_is_owner(mock_db, mock_user):
    ...
    with (
        ...
        patch(
            "app.services.book_service.update_book",
            return_value=fake_updated,
        ),
        patch("app.services.book_service.invalidate_book"),
    ):
        result = modify_book(
            mock_db, book_id=1, modified_data=book_data, current_user=mock_user
        )
...
def test_remove_book_succeeds_when_user_is_owner(mock_db, mock_user):
    ...
    with (
        ...
        patch("app.services.book_service.invalidate_book"),
        patch("app.services.book_service.delete_book"),
    ):
        remove_book(mock_db, book_id=1, current_user=mock_user)
